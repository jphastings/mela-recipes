...
// Recipe is a single recipe read out of a MealMaster archive, held as its mapped
// interchange form. The format is read-only: emitting MealMaster output is not
// supported.
type Recipe struct {
	filename string
...
func (r *Recipe) Marshal(io.Writer) error {
	return errors.New("writing MealMaster recipes is not supported")
}
