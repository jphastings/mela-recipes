...
// Recipe is a recipe parsed from schema.org structured data, held as its mapped
// interchange form. The format is read-only: emitting schema.org output is not
// supported yet.
type Recipe struct {
	filename string
...
func (r *Recipe) Marshal(io.Writer) error {
	return errors.New("writing schema.org recipes is not yet supported")
}
